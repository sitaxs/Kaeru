...
export default function BurgerMenu({ isOpen, onClose }: BurgerMenuProps) {
    ...
    const menuContent = (
        <div className="fixed inset-0 z-[9999] flex justify-end overflow-hidden">
            <div
                className={`absolute inset-0 bg-black/60 transition-opacity duration-500 ease-in-out ${isAnimating ? "opacity-100" : "opacity-0"
                    }`}
                onClick={onClose}
            ></div>

            <div
                className={`relative w-full md:w-[380px] h-[100dvh] bg-white shadow-[-10px_0_40px_rgba(0,0,0,0.08)] flex flex-col rounded-none md:rounded-l-[2rem] transition-transform duration-500 ease-in-out transform ${isAnimating ? "translate-x-0" : "translate-x-full"
                    }`}
            >

                <div className="flex items-center justify-between p-6 border-b border-gray-l">
                    <span className="text-xl font-medium tracking-widest text-m-t">KAERU</span>
                ...
                <div className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-2 scrollbar-hide font-medium">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <button onClick={() => handleNavigation('/profile/tickets')} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-gray-100 transition-colors text-m-t text-lg text-left group">
                            <span className="w-8 flex justify-center text-gray-d group-hover:text-orange transition-colors">
                               <TicketIcon/>             </span>
                            Мої квитки
                        </button>

                        <button onClick={() => handleNavigation('/reminders')} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-gray-100 transition-colors text-m-t text-lg text-left group">
                            <span className="w-8 flex justify-center text-gray-d group-hover:text-orange transition-colors">
                        ...
                        <button onClick={() => handleNavigation('/favorites')} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-gray-100 transition-colors text-m-t  text-lg text-left group">
                            <span className="w-8 flex justify-center text-gray-d group-hover:text-orange transition-colors">
                        ...
                        <button onClick={() => handleNavigation('/profile/studio')} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-grean-l/10 transition-colors text-m-t  text-lg text-left group">
