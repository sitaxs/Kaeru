...
export default function Header() {
  ...
  return (
    <>
      <div
        className={`hidden md:block fixed inset-0 bg-black/30 z-40 transition-opacity duration-500 ${
          isSearchExpanded ? "opacity-100 visible" : "opacity-0 invisible"
        }`}
        onClick={() => { setIsSearchExpanded(false); setActiveSearchTab(null); }}
      />

      {/* РОЗПІРКА: Ідеально вираховані пікселі + плавна анімація */}
      <div 
        aria-hidden="true"
        className={`w-full transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${
          collapsed 
            ? "h-[80px] md:h-[96px]"  // Точно 80px для моб, 96px для ПК (h-20 + pb-4)
            : "h-[80px] md:h-[210px]" // Точно 210px для ПК (h-20 + pb-6 + mt-4 + SearchBar 90px)
        }`}
      />

      {/* HEADER: Синхронізований duration-500 та ease */}
      <header
        className={`fixed top-0 z-50 w-full bg-white transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] border-b border-gray-200 ${
          collapsed 
            ? "pb-4 md:pb-4 shadow-sm" 
            : "md:border-transparent pb-4 md:pb-6"
        }`}
      >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-50 ">
          <div className="flex h-16 md:h-20 items-center justify-between relative">
            ...
            <search className="hidden md:flex flex-[2] lg:flex-[2.5] items-center justify-center relative h-[3.5rem] z-10 px-2 lg:px-4">
              <div
                className={`flex items-center justify-between border border-gray-l rounded-full shadow-sm hover:shadow-md bg-white transition-all duration-500 p-1.5 gap-1 w-full max-w-[650px] ${
          ...
          {/* ВИПАДНИЙ ПОШУК: Також синхронізовано час анімації */}
          <div
            className={`hidden md:flex transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] transform origin-top justify-center w-full ${
              collapsed ? "h-0 opacity-0 scale-y-75 pointer-events-none mt-0 absolute" : "h-[90px] opacity-100 scale-y-100 mt-4 relative"
            }`}
          >
            <div className="w-full transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] z-50">
              <SearchBar
